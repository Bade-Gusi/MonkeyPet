...
- 尺寸: 460x720，FixedDialog，居中，TopMost
...
- 行为勾选项: 始终置顶 / 启用叫声 / 显示操作提示
- 图片: 每只猴子可选透明 PNG（存路径到 settings.json，null 则用默认脸）
- 抠图工具: 颜色抠像（纯图片像素处理，不碰游戏进程）——选背景色 + 容差 + 边缘柔化，
  预览后保存为 1~4 号猴子；自动取四角平均色当默认背景色；最长边缩到 500px 处理
- 持久化: `%AppData%\MonkeyPet\settings.json`，改动即存，启动时加载
- 图标语义: 按钮使用 Emoji 图标 + 中文文字（🔊试听 ♻恢复 ✖退出 🐵1~4号）
