...
- 物品若卡在窗口矩形内（出生点/窗口盖上来），每帧向外一圈圈找最近的空路，
  朝那儿跑（Unstick）；卡住期间只允许穿越窗口逃出，**屏幕边界永远强制**，不会飞出屏幕

## Collision & Physics（碰撞物理，设置「碰撞物理」页可调）
- 碰撞体积: `CollisionSizePercent`（40~120%，默认 60）→ 同时决定撞窗口的盒子和撞物品的圆半径
- 物品互撞: `ItemCollisionEnabled`（默认开）——两两圆形弹性碰撞：推开 + 沿法线按弹力交换速度
- 弹力: `BounceElasticity`（0~100%，默认 60）→ 撞墙/互撞后保留的速度比例
