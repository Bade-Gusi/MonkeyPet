...
- 👨‍👩‍👧 右键点击猴子 → 定格 0.3 秒 + 放大 + 播放 `assets\dad.wav` 喊「爸爸」，
  ...
  （没有音频就用系统「哔哔」两声代替）
- 🗣 按 **F3**（或设置里「一起喊爸爸」/ API `/api/monkeys/roar-all`）让**所有猴子一起喊爸爸**
- 🎮 更多趣味玩法：
  - 左键戳猴子 → 它蹦一下「咦！」
  - 左键拖住猴子扔出去 → 它会滑行、撞窗口反弹、慢慢停下来
  - **F4** 一起跳舞 8 秒（设置里也有按钮）
  - **F5** 跟随鼠标走（再按一次关闭）
  - **B** 从天上扔根香蕉，猴子们抢着吃（吃到 +1 分、变大一圈）
  - 太久没人碰（45 秒）→ 猴子们趴下睡觉 💤，动一下鼠标就醒
...
不用手动找文件夹！在设置窗口「猴子图片」页点 **【打开素材文件夹】** 按钮，
...
| POST | `/api/monkeys/{id}/roar` | 让这只猴子喊爸爸（弹模拟消息框） |
| POST | `/api/monkeys/{id}/poke` | 戳一下（跳起来） |
| POST | `/api/monkeys/{id}/toss?vx=400&vy=-300` | 把这只猴子扔出去 |
| POST | `/api/monkeys/roar-all` | 让所有猴子一起喊爸爸 |
| POST | `/api/dance` | 所有猴子一起跳舞 |
